...
**Keep to budget.** A lane is sized for roughly thirty minutes of work. If you are approaching that and the task is not done, stop, commit what is coherent, and hand off with a precise state note (what is done, what remains, where you were) rather than pushing on. Never wait on a background process for more than five minutes; if a proof needs a long run, use a smaller fixture or report the run as not performed. When you must wait, wait on the process id (`wait $pid`, or `kill -0 $pid` in a bounded loop) or on a completion file the command writes; never loop on `pgrep -f "<pattern>"` or `ps | grep <pattern>`, because the pattern is part of your own polling shell's command line, the match never clears, and the loop runs forever. Prove on the smallest fixture that exercises the invariant; full-repository proofs are the coordinator's to schedule.
