...
Real `context plan` (2-file fixture, built binary):
```
...
```
Reuse path (second plan, same request, new actor):
```
notice      this plan's scope is incomplete and candidates were excluded with reasons; page them with `codectx context entries <session-id> --view excluded`
```
User config `context.max_seeds = 1` → `context entries --view excluded`:
```
      1  seed discovery stopped at the context.max_seeds bound of 1 while...
      2  seed discovery stopped at the context.max_seeds bound of 1 while...
```
A project `.codectx.toml` setting it is refused ("only the user configuration may set"), matching the `user` trust row.
...
- Reuse-path notice carries no count: the exclusion count is not in the manifest header and reading it would cost a store page.
