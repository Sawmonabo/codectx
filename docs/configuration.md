...
| `query_timeout` | `"10s"` | Deadline for one query. `codectx search` and `codectx symbol` apply it to the whole request, from pinning the generation to hydrating the page; exceeding it is `CTX_QUERY_DEADLINE`, an explicit incomplete answer, never a persisted complete one. This is the **default** deadline, taken by a call that carries none of its own; `--timeout` on the command line, or a deadline an MCP client attaches to its request, sets the deadline for that call instead — above this value as readily as below it. |
