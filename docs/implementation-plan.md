...
Use short transactions and prepared statements. No large transaction contains an entire analyzer run. The WAL writer opens with `PRAGMA synchronous = NORMAL` by default and `storage.synchronous = "full"` opts back into a per-commit fsync (ADR-0004); readers are pinned to `FULL`. Batching amortizes write synchronization. A future less-durable index-only mode must not silently weaken receipt durability. `quick_check`, full FTS integrity checking, VACUUM, and whole-CAS verification run on initialization, an explicit `doctor --deep`, recovery, or scheduled maintenance, not every short-lived query invocation; an ordinary `doctor` reads the header, the schema fingerprint and the journal mode and reports the content walk `unverified`.
