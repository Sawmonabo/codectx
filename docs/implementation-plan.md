...
without entering an API key or connecting to a hosted service. If only the bundled structural providers are available, the commands still work and report their precision. If SCIP, LSP, or Joern are installed locally, the same graph and query APIs become richer without changing the agent integration.
