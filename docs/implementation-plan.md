...
install.sh                                      (the POSIX sh installer, also served from the latest-release download path)
```
...
The pin-everything rule of this specification governs what the **product executes** -- the analyzer payloads, runtimes and grammars the lock names, every one of which is digest-pinned and never resolved as `latest`. It does not govern which *release of codectx* a person chooses to install: `install.sh` therefore supports `latest` and defaults to it, alongside a pinned `--version`, because the installer's integrity comes from verifying the downloaded archive against that release's own `checksums.txt` before extraction, not from the operator having typed a version number.
...
- [x] **Step 2: Implement the complete production path.** Implement metadata-only diagnostics, optional expensive deep checks and typed safe remediation. Track parent/base-worker/optional/full-tree memory distinctly with simultaneous sampling. Count queues, cached capacity, native resources, temporaries and DB/WAL. GC acquires the indexing/GC coordination lock so capture and collection cannot race; recheck leases/reachability before trash/final deletion. Prune finite closed-session audit data in dependency order. Recover abandoned staging and unreferenced temp artifacts without deleting retained source. *(`internal/diagnostics` and `internal/retention` landed with the `codectx doctor`, `codectx repo-map` and `codectx status --resources` commands behind them. Two figures in the Section 23 block are reported `unavailable` rather than measured, and deliberately: per-run unit reuse/parse counts are not process-wide totals, and a second process has no cross-process source for a watch's pending events because no watch heartbeat is persisted. Both are recorded in `docs/operations.md`'s unsupported-metric table. Recording the supplied `--scip-index` path against the generation **has landed**: `generation_supplied_indexes` records every supplied path and whether this generation holds the unit it produced, so `doctor`'s `supplied_index` check now tells "no index was supplied" from "the supplied path matched nothing". The watch heartbeat has **not**: it stays disclosed as `unavailable` with its reason in `docs/operations.md`'s unsupported-metric table, because a heartbeat that reports a dead process's last row as live coverage is a worse answer than the honest absence.)*
...
go test ./internal/graph ./internal/pagination ./internal/coverage ./internal/cli -count=1
go vet ./...
git diff --check
```

`go test ./internal/e2e` was struck from this block rather than left to fail:
that package does not exist at this task and is Task 21's to create. The line
above replaces it with the packages this task's rows actually landed in.
...
- [x] **Step 0: Complete the read, trace, reuse and resource gate.** Apply Section 30.1 to the actual current files and callers before deciding or editing. Record missing/unread context rather than guessing. Every subagent performs its own read.
- [x] **Step 1: Protect the critical behavior with the minimum existing test extension.** Use the one complete index/search/impact/plan/read/confirm/review/consolidate/capsule scenario through CLI and real MCP. Extend it for incremental mutation and old-session retained reads. The shared performance matrix checks memory/latency/reuse, and the offline run uses the compiled binary with all build dependencies prepared before isolation. *(`internal/e2e` is one package, one scenario: `TestE2ESearchParity` proves CLI and real MCP stdio answer from the same generation, and `TestE2EProductBoundary`'s four rows cover the walk through capsule, incremental mutation, the old session's retained read and the populated-capsule human rendering. Every row is mutation-proved against product code. `internal/bench`'s `TestResourceBudgets` is the shared matrix; `ci.yml`'s `offline` job builds, prefetches and configures before entering the network namespace.)*
- [ ] **Step 2: Implement the complete production path.** Measure Section 23 reference and low-memory workloads using release builds, including parser worker/native memory and full optional process tree; the benchmark corpora are cloned at pinned commits recorded with the results so they serve as the differential oracle for any future engine change. Validate exact capability/fact/query fingerprints before accepting an optimization. Use network-disabled Linux execution plus attempted-socket auditing where supported; no test downloads modules/grammars during isolated runtime. Build/test all six OS/arch combinations with compatible native toolchains; build the six bundle archives by running `codectx tools prefetch --all` into a fresh store per target and verify a bundle indexes the polyglot fixture with networking disabled. Run the Task 22 tools matrix as a release gate. Generate SBOM and native/module license inventory using pinned existing tools; do not write a new release framework. *(Open, honestly. Landed: the small-real measurement pass and the published benchmarks, the low-memory profile, capability/fact/query fingerprint parity, the network-disabled run with an attempted-socket audit, the >200-file session-status clamp row, the `max_graph_depth` re-pin (measured on a real workspace; the default stays 3), a cold index of the 10 000-file reference corpus inside the 3 min budget, `release.yml`'s six native legs with the Task 22 matrix as their gate, and a digest-pinned SBOM generator run for real. Not landed and named in `docs/performance.md`: the base storage ratio, which misses 3.5x by 21x on that corpus and whose per-row identity-width amplifier is diagnosed but not yet fixed; the reference corpus's line and byte size, ~3.8x and ~14x short of Section 23.1, so the cold-index figure stands for the file count only; the six bundles and two of the six build targets -- windows/arm64 needs a native aarch64 Windows toolchain and darwin needs a macOS SDK, so both are unproven on the measuring host and are built only by the release workflow's native legs; `codectx tools prefetch --all` into a fresh store; and the pinned third-party corpora, of which only this repository is recorded in `internal/bench/corpora.json`.)*
- [x] **Step 3: Wire consumers, failure handling and documentation.** Complete quick start, source/provenance limitations, strict actor workflow with receipts/review, managed toolchain behavior and offline/bundle use, configuration, retention/rebuild, diagnosis and performance evidence. Check all document anchors/cross-references and command/model parity. Report any unmet budget or unverified platform/profile as a blocker rather than declaring success. Stage/commit reviewed generated files before checking clean-tree status; do not require an empty diff before the release changes have been committed. *(`docs/performance.md` is the evidence page and carries the misses-and-unproven section; `README.md`, `SECURITY.md`, `CONTRIBUTING.md`, `docs/context-sessions.md`, `docs/toolchain.md` and `docs/queries.md` carry the rest.)*
- [x] **Step 4: Run the focused verification below.** These are commands to run during implementation, not claimed results of this document review. Diagnose failures, rerun after fixes, and record the actual result and resource evidence.
...
Every selector above now selects something: `internal/e2e` is one package with two
test functions, and `TestResourceBudgets` and the seven `Benchmark*` functions live
in `internal/bench/budgets_test.go`. Both packages skip entirely under `-short`,
because each row builds a real workspace and spawns real parser workers.
