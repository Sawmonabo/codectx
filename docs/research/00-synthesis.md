...
runs of engine 4.0.627 and all six SCIP indexers on this machine). Where a report and a real run
...
| Long-term backend | The engine remains viable once capped: 239k Go lines in 12 s / 1 GB, 506k Python in 37 s / 3.9 GB. One real engine bug (its JS/TS frontend crashing on a code shape) was hit on a 1.0M-line Meteor app and on its 324k-line client; per-unit runs contain it. No alternative covers nine languages with control and data dependence: Infer is compositional but C/Java/ObjC and emits no dependence graph; CodeQL is heavier; Fraunhofer cpg has the same JVM profile. Revisit only if the engine bug rate in the CI matrix stays high. |
...
## 1. Why peers feel instant and the engine does not
...
dependence. GitHub code-graph tools resolve calls by name ladders with ~80% coverage. The engine runs
...
CSV. Measured on codectx (37,590 Go lines): scip-go 0.65 s / 109 MB / 6.9 MB index; engine parse
...
| 2 | Reimplementing control dependence on tree-sitter | **Do not do it now.** Bounded algorithm (~1.2–1.8k LOC core) but 2.5–4.5k LOC of differential tests, a precision downgrade to `syntax`, and engine 4.0.627 covers all nine languages (Rust through its own frontend, needs cargo). | 05, 07 |
...
| 7 | The engine fully lazy | **Yes, extraction-lazy.** Overlays persist in cpg.bin; cache cpg per (unit, input hash, engine version); parse in the background after the base generation activates (`auto`), queried units first; never on the base-readiness path. `--nooverlays`/`--overlaysonly` two-phase is broken at 4.0.x. | 06, 07 |
| 8 | Bounded extraction vs full CSV | **Keep one `--repr=all --format=neo4jcsv` export.** the engine's slice command cannot emit CDG; DOT views are display-filtered projections. Measured export cost is acceptable (2 s, 49 MB for 37k lines). | 06, 07 |
...
The engine is engine 4.0.627, reached through one backend package; its product name appears in the
repository, the tool lock and the toolchain pages only. Unit = frontend-native project (Go module, Maven/Gradle module,
package.json or tsconfig project, Python package, Cargo package); C/C++ analyzed as one unit with
real include paths. One parse command with `--language <l>` per unit, one `--repr=all --format=neo4jcsv`
...
1. **Task 11 (before Task 12):** rename provider/package to `dependence` with
   the engine backend; always pass `--language`; one unit per frontend-native project, C/C++
...
5. **Task 22:** the engine's lock entry maps to capability `dependence` in `tools status` output; engine
