...
| R6 | **Type-recovery heuristics** | §2 lines 104-111 classes the 5,413 lines as "dead weight or serves `calls` in dynamic languages"; not sized anywhere in §7.5 | **contradicts** | the measured aggregate is 5,413 lines (1,331 generic + 4,082 per-language, of which a builtin table is 1,094); the figure **addable to an effort table for the product's six frontends is 4,885** (`16-passes-callgraph-and-types.md` §B9), because 1,628 of the 4,082 belong to frontends the product never invokes and the 1,331 omits 610 further shared lines the chain needs. The C, Go and Rust drivers do not override post-processing, and the Java override is gated on a flag the product never passes, so it is load-bearing for **two** of the product's six frontends — the ECMAScript family and Python — which are **95.24%** of the reference repository's call sites (526,393 + 488 + 2,291 of 555,588). It feeds no tool directly; it is what makes the `calls` those four tools serve resolvable |
...
| **4 — static call linking for the four families the engine gives no type recovery** | The static call linker, the method-reference linker and the attribution and stub passes — **361 measured Scala lines** — plus per-language full-name synthesis over the syntax tree. Native: resolved `calls` for C/C++, Go, Rust **and Java** — Java belongs here, not in phase 5, because its type-recovery override is gated on a flag the product never passes (`16-passes-callgraph-and-types.md` §B2), so the engine gives it a name join too | Resolved `calls` for **the ECMAScript family and Python, in units where no precise profile applies** — the two whose drivers actually run type recovery on the pinned argv | On a pinned corpus per language, the native `calls` key set matches the engine's **in-repo-resolved** subset within the per-family band. This phase must be gated on corpora and **not** on the reference repository, where it carries zero measured weight: that repository has no C, Go or Rust unit in either store | A full name is a frontend product, not a language property. Over a syntax tree the product mints its own, and a mismatch produces a **wrong** edge rather than a missing one — invisible to a consumer, and the same failure class already measured at 19% wrong call edges when per-file graphs were spliced |
| **5 — type recovery, per dynamic language** | The type-recovery fixed point and its per-language overrides for the two frontends that actually run them — **4,885 addable measured lines** for the six product frontends, of which a builtin table is 1,094 (the familiar 5,413 is the aggregate of two directories, 1,628 of it belonging to frontends the product never invokes; `16-passes-callgraph-and-types.md` §B9) — and the dynamic call linker, 226 lines. Order: JavaScript/TypeScript, then Python. Native: resolved `calls` for all nine languages | **Nothing** | Per language, both must hold: (a) the native `calls` key set reproduces the engine's **in-repo-resolved** subset within the per-family band on the pinned corpus; **and** (b) on a fresh index of the reference repository, a store query returns in-repo-resolved `calls` **≥ 40,743 sites / 27,897 edges** for the languages ported, while the ambiguous syntax-tier population does not rise above its measured 20,046 sites / 179,626 candidate edges | Type recovery's output is a heuristic fixed point: "within the band" can mean faithfully reproducing wrong edges. The gate is stated on the in-repo-resolved subset and **never** on the 177,138 external stub sites, because reproducing a stub reproduces no capability |
