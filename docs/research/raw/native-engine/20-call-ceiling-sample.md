...
| id | file:line | call expression | callee | syntax | engine | class | reason |
|---|---|---|---|---|---|---|---|
| C1-241 | `app/packages/mikemccrickard-ammap/web.browser/lib/plugins/export/libs/fabric.js/fabric.min.js`:7 | `this.getCurrentCharStyle(s,u)` | `getCurrentCharStyle` | unresolved | absent | b-lib | fabric.js prototype method in a vendored bundle |
| C1-242 | `app/packages/mikemccrickard-ammap/web.browser/lib/plugins/export/libs/pdfmake/pdfmake.js`:16132 | `baseAssignValue(result, iteratee(value, key, object), value)` | `baseAssignValue` | in-file | absent | b-lib | bundled lodash internal in a vendored pdfmake bundle |
...
| C1-244 | `app/packages/mikemccrickard-ammap/web.browser/lib/plugins/export/libs/pdfmake/pdfmake.js`:43843 | `feature('stylisticAlternatives', 'stylisticAltEleven')` | `feature` | in-file | absent | b-lib | in-file helper of a vendored pdfmake bundle |
| C1-245 | `app/packages/mikemccrickard-ammap/web.browser/lib/plugins/export/libs/pdfmake/pdfmake.js`:66619 | `__webpack_require__(142)` | `__webpack_require__` | in-file | absent | b-lib | webpack runtime require in a vendored bundle |
...
| C1-253 | `app/packages/mikemccrickard-ammap/web.browser/lib/plugins/export/libs/xlsx/xlsx.min.js`:10 | `px2pt(row.hpx)` | `px2pt` | in-file | absent | b-lib | in-file helper of a vendored xlsx bundle |
| C1-254 | `app/packages/sylido-meteor-selectize-bootstrap/selectize/dist/js/selectize.min.js`:1 | `e(g[d],c)` | `e` | in-file | absent | b-lib | minified local helper in a vendored selectize bundle |
| C1-255 | `app/server/actionButtons/actionButtons.js`:584 | `_.union(_.get(relatedAggsByType, k, []), val)` | `union` | unresolved | stub | b-lib | lodash union via the app-scope underscore global |
| C1-256 | `app/server/campaigns/campaigns.js`:247 | `_.get(auth, "sub._id")` | `get` | unresolved | stub | b-lib | lodash get via the app-scope underscore global |
...
| C1-258 | `app/server/fortressAPI/fortressAPI.js`:58 | `_.mapValues(_.get(requestOptions, "headers", {}), (v) => String(v))` | `mapValues` | unresolved | in-repo | b-lib | lodash mapValues via the app-scope underscore global |
| C1-259 | `app/server/jsreports/print.js`:174 | `_.get(user, "information.email", "")` | `get` | unresolved | stub | b-lib | lodash get via the app-scope underscore global |
...
| C1-261 | `app/server/lib/publish/myWatchedItems.js`:122 | `_.get(Meteor, "settings.flags.debug", false)` | `get` | unresolved | stub | b-lib | lodash get via the app-scope underscore global |
| C1-262 | `app/server/lib/publish/surveys.js`:813 | `_.escapeRegExp(query)` | `escapeRegExp` | unresolved | stub | b-lib | lodash escapeRegExp via the underscore global |
...
| C1-264 | `app/server/manageScoring/manageScoring.js`:416 | `_.get(oldDoc, "md.id")` | `get` | unresolved | stub | b-lib | lodash get via the app-scope underscore global |
| C1-265 | `app/server/manageUsers/manUsers.js`:430 | `Meteor.users.find({ _id : { $in : tenantUserIds }, "information.vendorPortal" : true, i…` | `find` | unresolved | stub | b-lib | find on the Meteor users Mongo collection |
...
| C1-267 | `app/server/navigation/navigation.test.js`:333 | `expect(boFindOne.mock.calls[0][0])` | `expect` | unresolved | absent | b-lib | jest global expect in a unit test |
| C1-268 | `app/server/rolesNotifications/rolesNotifications.js`:88 | `EmailTemplates.find($match, $project)` | `find` | unresolved | in-repo | b-lib | find on a Meteor Mongo.Collection instance |
| C1-269 | `app/server/serverRouteApi/serverRouteApi.js`:157 | `logger.error("(/fp-api/runAggsMulti) Missing header parameters.", { nonce, hashAuth })` | `error` | unresolved | in-repo | b-lib | winston logger level method on a global instance |
| C1-270 | `app/server/surveys/importExport.js`:286 | `_.get(auth, "user._id")` | `get` | unresolved | stub | b-lib | lodash get via the app-scope underscore global |
| C1-271 | `app/server/taxonomy/taxonomy.js`:97 | `taxonomySchema.validate(modifier, { modifier : true })` | `validate` | import | stub | b-lib | SimpleSchema validate on an aldeed schema instance |
| C1-272 | `app/server/unitTests/boAutomationRules/boAutomationRules.app-test.js`:96 | `sinon.createSandbox()` | `createSandbox` | import | absent | b-lib | sinon createSandbox |
| C1-273 | `app/server/unitTests/cmDashboard/cmDashboardFindingsGrid.app-test.js`:419 | `chai.expect(runChart({ vendorID : "" }))` | `expect` | import | absent | b-lib | chai expect |
| C1-274 | `app/server/utils/emailUtils.js`:572 | `_.get(val, "email", "")` | `get` | unresolved | stub | b-lib | lodash get via the app-scope underscore global |
...
| C1-276 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/bootstrap.js`:190 | `$(element)` | `$` | unresolved | stub | b-lib | jQuery factory call inside a vendored bootstrap |
| C1-277 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/inspinia.js`:255 | `$('body')` | `$` | unresolved | stub | b-lib | jQuery factory call in a vendored theme script |
| C1-278 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/jquery-2.1.1.js`:3 | `n.now()` | `now` | unresolved | absent | b-lib | jQuery internal now on the vendored jQuery build |
| C1-279 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/jquery-ui-1.10.4.min.js`:6 | `e("<a>")` | `e` | ambiguous | absent | b-lib | jQuery factory alias in vendored jQuery UI |
...
| C1-281 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/jquery-ui-1.10.4.min.js`:7 | `s.outerHeight()` | `outerHeight` | unresolved | absent | b-lib | jQuery outerHeight on a jQuery object |
| C1-282 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/jquery-ui-1.10.4.min.js`:7 | `this.helper.offset()` | `offset` | unresolved | absent | b-lib | jQuery offset on a jQuery object |
| C1-283 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/chartist/chartist.min.js`:7 | `c.serialize(o.meta)` | `serialize` | unresolved | absent | b-lib | Chartist util serialize in a vendored bundle |
| C1-284 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/codemirror/codemirror.js`:7629 | `getOrder(line)` | `getOrder` | in-file | in-repo | b-lib | in-file helper of the vendored CodeMirror core |
| C1-285 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/codemirror/mode/rst/rst.js`:170 | `stream.match(rx_role_pre, false)` | `match` | unresolved | in-repo | b-lib | CodeMirror StringStream match |
| C1-286 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/codemirror/mode/sparql/sparql.js`:121 | `pushContext(state, "}", stream.column())` | `pushContext` | in-file | in-repo | b-lib | in-file helper of a vendored CodeMirror mode |
| C1-287 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/codemirror/mode/xml/xml.js`:133 | `stream.match(/^[^\s\u00a0=<>\"\']*[^\s\u00a0=<>\"\'\/]/)` | `match` | unresolved | in-repo | b-lib | CodeMirror StringStream match |
| C1-288 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/cropper/cropper.min.js`:9 | `this.renderImage("zoom")` | `renderImage` | unresolved | absent | b-lib | cropper plugin method on its own instance |
| C1-289 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/dataTables/jquery.dataTables.js`:500 | `_fnCompatMap( init, 'orderDataType', 'sortDataType' )` | `_fnCompatMap` | in-file | in-repo | b-lib | in-file helper of vendored DataTables |
| C1-290 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/dataTables/jquery.dataTables.js`:4217 | `tmpTable.css( 'width', 'auto' )` | `css` | unresolved | in-repo | b-lib | jQuery css on a jQuery object |
...
| C1-292 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/footable/footable.all.min.js`:14 | `t(a.table).unbind(".sorting").bind({"footable_initialized.sorting":function(){var i,o,n…` | `data` | unresolved | absent | b-lib | jQuery data on a jQuery object in vendored FooTable |
| C1-293 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/fullcalendar/fullcalendar.min.js`:6 | `ye(r,t[s],r.forwardSegs)` | `ye` | in-file | absent | b-lib | minified in-file helper of vendored FullCalendar |
| C1-294 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/fullcalendar/moment.min.js`:6 | `r(C(a)%60,2)` | `r` | in-file | absent | b-lib | minified in-file helper of vendored moment |
| C1-295 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jqGrid/jquery.jqGrid.min.js`:117 | `b(y,a).closest("table.ui-jqgrid-btable").attr("id")` | `attr` | unresolved | absent | b-lib | jQuery attr on a jQuery object |
| C1-296 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jqGrid/jquery.jqGrid.min.js`:221 | `a(c)` | `a` | ambiguous | absent | b-lib | jQuery factory alias in vendored jqGrid |
| C1-297 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jqGrid/jquery.jqGrid.min.js`:332 | `a("#"+v.themodal)` | `a` | ambiguous | absent | b-lib | jQuery factory alias in vendored jqGrid |
| C1-298 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jqGrid/jquery.jqGrid.min.js`:411 | `a.isFunction(b.p.beforeSelectRow)` | `isFunction` | unresolved | absent | b-lib | jQuery isFunction utility |
| C1-299 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jqGrid/jquery.jqGrid.min.js`:437 | `s("unbind")` | `s` | ambiguous | absent | b-lib | minified local of the vendored jqModal plugin |
| C1-300 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jqGrid/jquery.jqGrid.min.js`:462 | `f.hasClass(a)` | `hasClass` | unresolved | absent | b-lib | jQuery hasClass on a jQuery object |
| C1-301 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jqGrid/jquery.jqGrid.min.js`:517 | `this.toObj(g)` | `toObj` | unresolved | absent | b-lib | xmlJsonClass method in the vendored jqGrid bundle |
...
| C1-303 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jquery-ui/jquery-ui.js`:6413 | `$( this )` | `$` | unresolved | stub | b-lib | jQuery factory call in vendored jQuery UI |
| C1-304 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jquery-ui/jquery-ui.js`:6605 | `toShow .hide()` | `hide` | unresolved | absent | b-lib | jQuery hide on a jQuery object |
| C1-305 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jquery-ui/jquery-ui.js`:8129 | `this._updateAlternate(inst)` | `_updateAlternate` | unresolved | in-repo | b-lib | jQuery UI datepicker internal method |
| C1-306 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jquery-ui/jquery-ui.min.js`:5 | `t(n.containment)` | `t` | in-file | absent | b-lib | jQuery factory alias in vendored jQuery UI |
| C1-307 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jquery-ui/jquery-ui.min.js`:8 | `s.removeClass("ui-accordion-header-active ui-state-active")` | `removeClass` | unresolved | absent | b-lib | jQuery removeClass on a jQuery object |
| C1-308 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jquery-ui/jquery-ui.min.js`:8 | `this._updateDatepicker(e)` | `_updateDatepicker` | unresolved | absent | b-lib | jQuery UI datepicker internal method |
| C1-309 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/jsTree/jstree.min.js`:4 | `c.element.find("ul:visible").addBack()` | `addBack` | unresolved | absent | b-lib | jQuery addBack on a jQuery object |
| C1-310 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/morris/raphael-2.1.0.min.js`:10 | `bJ(b)` | `bJ` | in-file | absent | b-lib | minified in-file helper of vendored Raphael |
...
| C1-312 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/rickshaw/rickshaw.min.js`:2 | `d3.svg.line().x(function(d){return graph.x(d.x)}).y(function(d){return graph.y(d.y)}).i…` | `tension` | unresolved | absent | b-lib | d3 line generator tension in vendored Rickshaw |
| C1-313 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/rickshaw/vendor/d3.v3.js`:1 | `u()` | `u` | ambiguous | absent | b-lib | minified in-file helper of vendored d3 v3 |
...
| C1-315 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/sparkline/jquery.sparkline.min.js`:4 | `e.get("colorMap")` | `get` | unresolved | absent | b-lib | sparkline internal options object get |
| C1-316 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/staps/jquery.steps.min.js`:6 | `h.eq(e)` | `eq` | unresolved | absent | b-lib | jQuery eq on a jQuery object |
| C1-317 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/sweetalert/sweetalert.min.js`:1 | `l.addClass(o,"visible")` | `addClass` | unresolved | absent | b-lib | sweetalert internal dom utils addClass |
| C1-318 | `mockup/INSPINIA-bootstrap-template/Static_Full_Version/js/plugins/validate/jquery.validate.min.js`:4 | `a(b).rules()` | `rules` | unresolved | absent | b-lib | jQuery validate plugin rules on a jQuery object |
| C1-319 | `mockup/INSPINIA-bootstrap-template/Static_Seed_Project/js/bootstrap.js`:1812 | `$tip.find('.popover-content')` | `find` | unresolved | stub | b-lib | jQuery find on a jQuery object |
| C1-320 | `mockup/INSPINIA-bootstrap-template/Static_Seed_Project/js/bootstrap.min.js`:6 | `c.isInStateTrue()` | `isInStateTrue` | unresolved | absent | b-lib | bootstrap button plugin internal method |
| C1-321 | `mockup/INSPINIA-bootstrap-template/Static_Seed_Project/js/jquery-ui-1.10.4.min.js`:6 | `this.headers.removeClass("ui-accordion-header ui-accordion-header-active ui-helper-rese…` | `removeAttr` | unresolved | absent | b-lib | jQuery removeAttr on a jQuery object |
...
| C1-323 | `mockup/INSPINIA-bootstrap-template/Static_Seed_Project/js/plugins/pace/pace.min.js`:2 | `t(p)` | `t` | unresolved | absent | b-lib | minified in-file helper of vendored pace |
| C1-324 | `mockup/INSPINIA-bootstrap-template/Static_Seed_Project/js/plugins/slimscroll/jquery.slimscroll.min.js`:11 | `b.scrollTop()` | `scrollTop` | unresolved | absent | b-lib | jQuery scrollTop on a jQuery object |
| C1-325 | `mockup/www/js/bootstrap.min.js`:6 | `a(this)` | `a` | unresolved | absent | b-lib | jQuery factory alias in vendored bootstrap |
| C1-326 | `mockup/www/js/bootstrap.min.js`:7 | `a(document).on("click.bs.tab.data-api",'[data-toggle="tab"]',e).on("click.bs.tab.data-a…` | `on` | unresolved | absent | b-lib | jQuery on for a delegated data-api handler |
| C1-327 | `mockup/www/js/jquery-ui-1.10.4.min.js`:6 | `this.focusable.add(e)` | `add` | unresolved | absent | b-lib | jQuery add on a jQuery collection |
| C1-328 | `mockup/www/js/jquery-ui-1.10.4.min.js`:6 | `this._hideDatepicker()` | `_hideDatepicker` | unresolved | absent | b-lib | jQuery UI datepicker internal method |
| C1-329 | `mockup/www/js/jquery-ui-1.10.4.min.js`:7 | `this._trigger("beforeActivate",e,c)` | `_trigger` | unresolved | absent | b-lib | jQuery UI widget _trigger |
| C1-330 | `mockup/www/js/plugins/amcharts/amcharts.js`:359 | `d.applyTheme(this,a,this.cname)` | `applyTheme` | unresolved | absent | b-lib | AmCharts applyTheme on the AmCharts namespace |
| C1-331 | `mockup/www/js/plugins/amcharts/funnel.js`:24 | `q.getBBox()` | `getBBox` | unresolved | absent | b-lib | AmCharts SVG wrapper getBBox; it also has translate |
| C1-332 | `mockup/www/js/plugins/amcharts/plugins/animate/animate.js`:288 | `getKeysSliced( chart, keys, seen )` | `getKeysSliced` | in-file | absent | b-lib | in-file helper of the vendored amcharts animate plugin |
| C1-333 | `mockup/www/js/plugins/amcharts/plugins/export/export.min.js`:1 | `c.isTainted(a)` | `isTainted` | unresolved | absent | b-lib | amcharts export plugin internal isTainted |
...
| C1-335 | `mockup/www/js/plugins/amcharts/plugins/export/libs/fabric.js/fabric.js`:21986 | `this.fire('editing:exited')` | `fire` | unresolved | absent | b-lib | fabric Observable fire on a fabric object |
| C1-336 | `mockup/www/js/plugins/amcharts/plugins/export/libs/pdfmake/pdfmake.js`:779 | `hexWrite(this, string, offset, length)` | `hexWrite` | in-file | absent | b-lib | in-file buffer shim helper in vendored pdfmake |
...
| C1-338 | `mockup/www/js/plugins/amcharts/plugins/export/libs/pdfmake/pdfmake.js`:15481 | `pack(ALPHANUMERIC_MAP[data.charAt(i-1)], 6)` | `pack` | ambiguous | absent | b-lib | in-file qrcode helper of the vendored pdfmake bundle |
| C1-339 | `mockup/www/js/plugins/amcharts/plugins/export/libs/pdfmake/pdfmake.min.js`:8 | `_(e,ai.placeholder)` | `_` | ambiguous | absent | b-lib | bundled lodash in a vendored pdfmake bundle |
| C1-340 | `mockup/www/js/plugins/amcharts/plugins/export/libs/pdfmake/pdfmake.min.js`:14 | `t.readInt()` | `readInt` | unresolved | absent | b-lib | internal stream reader of vendored pdfmake |
...
| C1-345 | `mockup/www/js/plugins/amcharts/plugins/export/libs/xlsx/xlsx.min.js`:2 | `write_num("n",r[1],ff[1])` | `write_num` | unresolved | absent | b-lib | internal writer of the vendored xlsx bundle |
...
| C1-347 | `mockup/www/js/plugins/fullcalendar/moment.min.js`:6 | `vb(this)` | `vb` | unresolved | absent | b-lib | minified in-file helper of vendored moment |
| C1-348 | `mockup/www/js/plugins/ionRangeSlider/ion.rangeSlider.js`:989 | `this.checkDiapason(this.coords.p_single_real, this.options.from_min, this.options.from_…` | `checkDiapason` | unresolved | absent | b-lib | ion.rangeSlider plugin method on its own instance |
| C1-349 | `mockup/www/js/plugins/ionRangeSlider/ion.rangeSlider.min.js`:50 | `this.callOnChange()` | `callOnChange` | unresolved | absent | b-lib | ion.rangeSlider plugin method on its own instance |
| C1-350 | `mockup/www/js/plugins/morris/morris.js`:1649 | `$.extend({}, this.defaults, options)` | `extend` | unresolved | absent | b-lib | jQuery extend utility |
...
| C1-352 | `mockup/www/js/plugins/slimscroll/jquery.slimscroll.min.js`:12 | `e("<div></div>").addClass(a.wrapperClass).css({position:"relative",overflow:"hidden",wi…` | `css` | unresolved | absent | b-lib | jQuery css on a constructed jQuery object |
...
| C1-354 | `mockup/www/js/plugins/switchery/switchery.js`:1 | `this.needsClick(this.targetElement)` | `needsClick` | unresolved | absent | b-lib | fastclick internal needsClick in vendored switchery |
| C1-355 | `mockup/www/js/plugins/typeahead/bloodhound.js`:61 | `$.each(obj, function(key, val) { if (result = test.call(null, val, key, obj)) { return …` | `each` | unresolved | absent | b-lib | jQuery each utility |
| C1-356 | `mockup/www/js/plugins/typeahead/bloodhound.js`:701 | `this.index.reset()` | `reset` | unresolved | absent | b-lib | bloodhound SearchIndex reset |
...
| C1-361 | `mongo scripts/internalTab-migration-initial.js`:749 | `_.each(_.get(selOption, "options", []), (option) => { newStatuses[_.toLower(_.get(optio…` | `each` | unresolved | absent | b-lib | lodash each, loaded into the shell by load() |
...
| C1-365 | `playwright-validation/sweep-session.js`:121 | `page.locator(".slick-row").first()` | `first` | unresolved | in-repo | b-lib | Playwright Locator first on a page locator |
| C1-366 | `vendorPortal/client/accounts/accountsTemplates.app-test.js`:76 | `chai.assert.isFalse(routerGo.calledWith("login"), "must not fall back to login on succe…` | `isFalse` | unresolved | absent | b-lib | chai assert isFalse |
| C1-367 | `vendorPortal/client/components/boCards/boAttachFileDialog.js`:132 | `_.get(tpl, "data.bo")` | `get` | unresolved | in-repo | b-lib | lodash get via the app-scope underscore global |
| C1-368 | `vendorPortal/client/lib/bootstrap-editable/js/bootstrap-editable.js`:1257 | `this.hide()` | `hide` | unresolved | in-repo | b-lib | bootstrap-editable plugin method on its own instance |
...
| C1-371 | `vendorPortal/client/lib/jquery-stickytableheaders/jquery.stickytableheaders.min.js`:1 | `f.css("padding-right")` | `css` | unresolved | absent | b-lib | jQuery css on a jQuery object |
| C1-372 | `vendorPortal/client/lib/jquery-ui-1.12.0.custom/jquery-ui.js`:2543 | `c.css( "paddingLeft" )` | `css` | unresolved | stub | b-lib | jQuery css on a jQuery object |
| C1-373 | `vendorPortal/client/pages/findings/findings.js`:185 | `Meteor.call("boUpdateSeen", _id)` | `call` | unresolved | stub | b-lib | Meteor.call on the Meteor namespace |
| C1-374 | `vendorPortal/client/plugins/blueimp/jquery.blueimp-gallery.min.js`:1 | `define(["./blueimp-helper","./blueimp-gallery"],a)` | `define` | unresolved | absent | b-lib | AMD define in a vendored blueimp bundle |
| C1-375 | `vendorPortal/client/plugins/d3/d3.min.js`:1 | `Math.cos(w)` | `cos` | unresolved | absent | c-platform | Math.cos built-in |
| C1-376 | `vendorPortal/client/plugins/d3/d3.min.js`:1 | `n.point(p[0],p[1])` | `point` | unresolved | absent | b-lib | d3 stream listener point in the vendored d3 build |
| C1-377 | `vendorPortal/client/plugins/d3/d3.min.js`:2 | `Su(r=l,u)` | `Su` | in-file | absent | b-lib | minified in-file helper of vendored d3 |
...
| C1-380 | `vendorPortal/client/plugins/d3/d3.min.js`:5 | `q.on("mousemove.brush",null).on("mouseup.brush",null)` | `on` | unresolved | absent | b-lib | d3 selection on for brush event handlers |
...
| C1-383 | `vendorPortal/packages/keithcoach-bootstrap3-datepicker/lib/js/bootstrap-datepicker.js`:500 | `this._detachEvents()` | `_detachEvents` | unresolved | in-repo | b-lib | bootstrap-datepicker plugin internal method |
| C1-384 | `vendorPortal/packages/matomo-custom/client/matomo.js`:7 | `FlowRouter.current()` | `current` | unresolved | stub | b-lib | ostrio flow-router-extra FlowRouter.current |
| C1-385 | `vendorPortal/packages/meteor-template-extension/lib/template-inherits-hooks-from.js`:16 | `self.onCreated(hook)` | `onCreated` | unresolved | stub | b-lib | Blaze Template onCreated on a template |
...
| C1-387 | `vendorPortal/public/okta-auth-js.min.js`:8 | `n.n(i)` | `n` | unresolved | absent | b-lib | webpack runtime helper in a vendored okta bundle |
...
| C1-389 | `vendorPortal/public/okta-auth-js.min.js`:8 | `n(9231)` | `n` | ambiguous | absent | b-lib | webpack module require in a vendored okta bundle |
| C1-390 | `vendorPortal/server/lib/publish/surveys.js`:456 | `_.get(v, "invited.o.rescindedUser", [])` | `get` | unresolved | in-repo | b-lib | lodash get via the app-scope underscore global |
