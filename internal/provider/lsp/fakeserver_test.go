...
//	callHierarchy/outgoingCalls      one callee in util.go with a call site on
//	                                 the queried item's own line in main.go ("y }")
...
func (f *fakeServer) handle(msg message) int {
	...
	case "callHierarchy/outgoingCalls":
		...
		var it callHierarchyItem
		json.Unmarshal(p.Item, &it)
		// The callee lives in the *other* fixture file and the call site in
		// the queried item's own document, so the two ends of the edge are
		// distinguishable: an implementation that attributed the sites to the
		// peer would validate main.go's coordinates against util.go.
		calleeURI := "file://" + filepath.ToSlash(filepath.Join(filepath.Dir(uriPath(it.URI)), "util.go"))
		funcs := f.functions(calleeURI)
		if len(funcs) == 0 {
			// Never panic on an empty slice: a dead child would look like a
			// provider failure rather than a missing fixture file.
			f.reply(msg, nil, &rpcError{Code: -32803, Message: "no function in " + calleeURI})
			return -1
		}
		...
		f.reply(msg, []any{map[string]any{"to": f.item(calleeURI, funcs[0]), "fromRanges": []any{site.rng}}}, nil)
